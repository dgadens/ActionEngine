...
HRESULT InitWindow( HINSTANCE hInstance, int nCmdShow )
{
    ...
    wcex.hIcon = LoadIcon( hInstance, ( LPCTSTR )IDI_SILENTDIMPROJECT );
    wcex.hCursor = LoadCursor( nullptr, IDC_ARROW );
    wcex.hbrBackground = ( HBRUSH )( COLOR_WINDOW + 1 );
    wcex.lpszMenuName = nullptr;
    wcex.lpszClassName = L"DemoWindow";
    wcex.hIconSm = LoadIcon( wcex.hInstance, ( LPCTSTR )IDI_SILENTDIMPROJECT );
    if( !RegisterClassEx( &wcex ) )
        return E_FAIL;

    // Create window
    HInst = hInstance;
    RECT rc = { 0, 0, SD_Globals::MainWndWidth, SD_Globals::MainWndHeight };
...
LRESULT CALLBACK WndProc( HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam )
{
    ...
    switch( message )
    {
		...
		case WM_MOUSEMOVE:
			SD_Globals::pMouse->SetPosition(LOWORD (lParam), HIWORD (lParam));
			break;
		case WM_LBUTTONDOWN:
			SD_Globals::pMouse->SetButtonDown(ACMOUSEBUTTON::ACMOUSE_LEFT);
			break;
		case WM_RBUTTONDOWN:
			SD_Globals::pMouse->SetButtonDown(ACMOUSEBUTTON::ACMOUSE_RIGHT);
			break;
		case WM_MBUTTONDOWN:
			SD_Globals::pMouse->SetButtonDown(ACMOUSEBUTTON::ACMOUSE_MIDDLE);
			break;

		case WM_LBUTTONUP:
			SD_Globals::pMouse->SetButtonUp(ACMOUSEBUTTON::ACMOUSE_LEFT);
			break;
		case WM_RBUTTONUP:
			SD_Globals::pMouse->SetButtonUp(ACMOUSEBUTTON::ACMOUSE_RIGHT);
			break;
		case WM_MBUTTONUP:
			SD_Globals::pMouse->SetButtonUp(ACMOUSEBUTTON::ACMOUSE_MIDDLE);
			break;
		...
		case WM_KEYDOWN:
			...
			//if (!Globals::ShowConsole)
				SD_Globals::pKeyboard->SetKeyDown((ACKEYS)wParam);
			break;
		case WM_KEYUP:
			//if (!Globals::ShowConsole)
				SD_Globals::pKeyboard->SetKeyUp((ACKEYS)wParam);
			break;
		...
		case WM_EXITSIZEMOVE:
			...
			SD_Globals::MainWndWidth = rcWnd.right - rcWnd.left;
			SD_Globals::MainWndHeight = rcWnd.bottom - rcWnd.top;

			pGame->ReziseWindow(SD_Globals::MainWndWidth, SD_Globals::MainWndHeight);
			pGame->Active(TRUE);
			return 0;
		...
		case WM_ACSERVER: case WM_ACCLIENT: 
			{
				HRESULT hr = SD_Globals::pNetworkDevice->MsgProc(wParam, lParam);
				if (FAILED(hr)) 
				{
...
HRESULT EngineStartup()
{
	...
	SD_Globals::pRenderDevice = pRenderer->GetDevice();
	if (SD_Globals::pRenderDevice == nullptr) 
		return E_FAIL;
	if (FAILED(SD_Globals::pRenderDevice->Init(HWnd, false, true)))
		return AC_FAIL;
	//ativa a janela principal
	SD_Globals::pRenderDevice->SetActiveViewport(HWnd);
	#pragma endregion
	...
	SD_Globals::pInputDevice = pInput->GetDevice();
	if (SD_Globals::pInputDevice == nullptr) 
		return E_FAIL;
	pInput->CreateMouse();
	SD_Globals::pMouse = pInput->GetMouse();
	pInput->CreateKeyboard();
	SD_Globals::pKeyboard = pInput->GetKeyboard();
	SD_GameInput::Init(SD_Globals::pKeyboard, SD_Globals::pMouse);
	#pragma endregion
	...
	//	return E_FAIL;
	//}
	//SD_Globals::pNetworkDevice = pNetwork->GetDevice();
	//if (SD_Globals::pNetworkDevice == nullptr) 
	//	return E_FAIL;
	#pragma endregion
	...
	SD_Globals::pAudioDevice = pAudio->GetDevice();
	if (SD_Globals::pAudioDevice == nullptr) 
		return E_FAIL;
	if (FAILED(SD_Globals::pAudioDevice->Init(HWnd, true)))
		return AC_FAIL;

	//configura e carrega alguns sons
	SD_Globals::pAudioDevice->SetListener(Vector3(0,0,0),Vector3(0,0,0),Vector3(0,0,0),Vector3(0,0,0));
	#pragma endregion

	#pragma region CRIA O CONTENT MANAGER
	pContentManager = new ACContentManager(SD_Globals::pRenderDevice, SD_Globals::pAudioDevice);
	#pragma endregion

	#pragma region CRIA O GAME
	pGame = new Game(SD_Globals::pRenderDevice, pContentManager);
	pGame->Initialize();
	pGame->Active(TRUE);
	pGame->LoadContents();
	#pragma endregion
